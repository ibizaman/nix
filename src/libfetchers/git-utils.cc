...
struct GitDirectorySinkImpl : merkle::DirectorySinkWithFlush
{
    ...
    Pool<GitRepoImpl>::Handle repo;
    Directory children;

    GitDirectorySinkImpl(Pool<GitRepoImpl>::Handle repo)
        : repo(std::move(repo))
    {
    }
...
{
    return make_ref<GitDirectorySinkImpl>(getOrCreatePool().get());
}
...
{
    auto handle = getOrCreatePool().get();
    git_oid oid;
